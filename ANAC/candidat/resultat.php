...
<!-- IF THÉORIE : résultat de passage uniquement -SCORE MASQUÉ (directive DG) -->
<div class="circle mx-auto" style="background:linear-gradient(135deg,#03224c,#0a3a6b);">
  <div style="font-size:2.2rem;margin-bottom:4px;">🔒</div>
  <div class="cs" style="font-size:.75rem;color:rgba(255,255,255,.85);text-align:center;line-height:1.4;">Score<br>confidentiel</div>
</div>
<div class="mention mt-2" style="color:<?= $ins?'#e03a3a':'#19a96b' ?>">
  <?= $ins ? '⚠️ Score insuffisant -Pratique non accessible' : '✅ Théorie validée -Accès à la Pratique autorisé' ?>
</div>
<div class="ai text-start mt-3">
  <i class="fas fa-info-circle me-2"></i>
  <?= $ins
    ? 'Votre score théorique est inférieur au seuil requis. Vous ne pouvez pas accéder à l\'épreuve pratique. Contactez l\'administration ANAC GABON.'
    : 'Votre épreuve théorique est validée. Veuillez passer à la <strong>salle de pratique</strong> après 15 minutes de pause obligatoire.' ?>
</div>
<div class="ai text-start mt-2" style="background:#fffbeb;border-color:#fde68a;">
  <i class="fas fa-lock me-2" style="color:#D4AF37;"></i>
  Conformément à la <strong>directive du DG ANAC GABON</strong>, votre score n'est pas communiqué directement. Votre résultat sera transmis à l'administration.
</div>

<?php elseif ($est_pr&&$pct_theo!==null&&$pct_prat!==null): ?>
<!-- IF PRATIQUE FINAL : résultat ADMIS/AJOURNÉ uniquement -SCORE MASQUÉ (directive DG) -->
<div class="circle mx-auto" style="background:linear-gradient(135deg,#03224c,#0a3a6b);">
  <div style="font-size:2.2rem;margin-bottom:4px;">🔒</div>
  <div class="cs" style="font-size:.75rem;color:rgba(255,255,255,.85);text-align:center;line-height:1.4;">Score<br>confidentiel</div>
</div>
<div class="mention mt-2" style="color:<?= $ok?'#28a745':'#dc3545' ?>">
  <?= $ok ? '✅ ADMIS(E) -CERTIFICATION IF' : '❌ NON ADMIS(E) -CERTIFICATION IF' ?>
</div>
<p class="lead text-muted mt-1"><?= $ok ? __('felicitations') : __('essayez_encore') ?></p>
<div style="background:#f0f4fa;border:1.5px solid #c7d2fe;border-radius:12px;padding:14px 18px;margin:12px 0;text-align:left;">
  <p style="font-weight:700;color:#03224c;margin-bottom:6px;font-size:.9rem;">
    <i class="fas fa-lock" style="color:#D4AF37;margin-right:7px;"></i>
    Score confidentiel -Directive OACI
  </p>
  <p style="font-size:.83rem;color:#4b5563;line-height:1.6;margin:0;">
    Vos notes de théorie, de pratique et votre moyenne IF ne sont pas communiquées
    directement. Conformément à la politique de l'OACI, votre résultat complet
    est transmis à l'administration ANAC GABON. Vous serez informé(e) par votre hiérarchie.
  </p>
</div>

<?php elseif ($is_form_final&&!empty($det_mods)): ?>
<!-- FORM FINAL : note/100 + pourcentage -->
<div class="circle mx-auto" style="background:linear-gradient(135deg,<?= $ok?'#28a745,#20c997':'#dc3545,#c82333' ?>);">
<div class="cn"><?= round($moy_form,1) ?></div>
<div class="cs">/ 100 pts</div>
<div class="cp"><?= round($moy_form,1) ?>%</div>
</div>
...
<!-- ══ SENS : score VISIBLE ══ -->
<div class="circle mx-auto" style="background:linear-gradient(135deg,<?= $ok?'#28a745,#20c997':'#dc3545,#c82333' ?>);">
<div class="cn"><?= round($nb,1) ?></div>
<div class="cs">/ <?= round($ns,1) ?> pts</div>
<div class="cp"><?= round($pct,1) ?>%</div>
</div>
...
<?php else: ?>
<!-- ══ AS / IF / INST : score CONFIDENTIEL ══ -->
<div class="circle mx-auto" style="background:linear-gradient(135deg,#03224c,#0a3a6b);">
  <div style="font-size:2.2rem;margin-bottom:4px;">🔒</div>
  <div class="cs" style="font-size:.78rem;color:rgba(255,255,255,.85);text-align:center;line-height:1.4;">
    Résultat<br>confidentiel
  </div>
</div>
<div class="mention mt-2" style="color:<?= $ok?'#28a745':'#dc3545' ?>">
  <?= $ok ? '✅ ADMIS(E)' : '❌ AJOURNÉ(E)' ?>
</div>
<p class="lead text-muted mt-1">
  <?= $ok ? __('felicitations') : __('essayez_encore') ?>
</p>
<!-- Encadré confidentiel explicatif -->
<div style="background:#f0f4fa;border:1.5px solid #c7d2fe;border-radius:12px;
            padding:14px 18px;margin:12px 0;text-align:left;">
  <p style="font-weight:700;color:#03224c;margin-bottom:6px;font-size:.9rem;">
    <i class="fas fa-lock" style="color:#D4AF37;margin-right:7px;"></i>
    Score confidentiel -ANAC GABON
  </p>
  <p style="font-size:.83rem;color:#4b5563;line-height:1.6;margin:0;">
    Conformément à la politique de l'ANAC GABON, votre note et votre
    pourcentage ne sont pas communiqués directement. Votre résultat a été
    transmis à l'administration. Vous serez informé(e) par votre hiérarchie
    ou votre service RH.
  </p>
</div>
<?php endif; ?>
...
<?php /* IF : PAS de récapitulatif avec notes -directive DG ANAC (confidentialité AS/IF/INST) */ ?>
